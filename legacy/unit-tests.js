var callback = (function($){
	$(function(){
		...
		test("negative exponents",function(){
			equal(formatter.format("{0:#.###}",0.0000000000005),"");
			equal(formatter.format("{0:0.000}",0.0000000000005),"0.000");
		});
		test("zeros in format after pounds",function(){
			equal(formatter.format("{0:0.##0}",0.1),"0.100");
			equal(formatter.format("{0:0.0##}",0.1),"0.1");
		});
		test("zeros in placeholder",function(){
			equal(formatter.format("{0:#.###}",0.012),".012");	
			equal(formatter.format("{0:#}",0.6),"1");		
			equal(formatter.format("{0:#.###}",0),"");
			//equal(formatter.format("{0:###.###}",0),"");
			//equal(formatter.format("{0:0.000}",0),"0.000");
			//equal(formatter.format("{0:000.000}",0),"000.000");
		});
		test("positive exponents",function(){
			for(var i = 0; i < 30 ; i ++)
			{
					var myString = "5" + new Array(i).join("0"),myInt = parseInt(myString);
					equal(formatter.format("{0:0}",myInt),myString);
			}
			equal(formatter.format("{0:0}",12463452345000000000000),"12463452345000000000000");
			equal(formatter.format("{0:0}",34513452573842312456745),"34513452573842310000000");
		});
		test("format negative",function(){
			equal(formatter.format("{0}",-1234),"-1234");
			equal(formatter.format("{0:#.000}",-1234),"-1234.000");
			equal(formatter.format("{0:#.####}",-0.1234),"-0.1234");
			equal(formatter.format("{0:#.###}",-0.0000005),"");
			equal(formatter.format("{0:#.###}",-0),"");
		});
		...
		test("pass in object",function(){
			...
			equal(formatter.format("{text} x {number:#,###.0000}",{
				text : "astral projection",
				number : 1234.5678
