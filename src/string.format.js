...
if(typeof String.prototype.format == "undefined")
{
	var utility = {
		...
		count : function (stringOfItemsToLookFor,whatToCount,useEscapeCharacters){
			...
			var result = 0;
			for(var i = 0; i < whatToCount.length; i++){
	...
	formatter.prototype = {
		format : function(format,value){
			...
			if(value instanceof Date) {
				...
				format = format.length == 1 && format in this.dateShorts ? this.dateShorts[format] : format;
				var result = [];
				...
				for(var i = 0; i < format.length; i++){
					...
					// do format
					var pushed = false;
					for(var j = 0; j < this.dateNames.length; j++){
						var d = this.dateNames[j];
				...
				if(value instanceof Number)
					value = value.valueOf();
				var shortcut = format.match(this.numberShortRegex);
				if(shortcut){
		...
		customNumberFormat : function(value,format){
			...
			if(format.indexOf("%") > -1){
				var percentages = utility.count("%",format);
				value = value * Math.pow(100,percentages);
			}
			
			var mil = "‰";
			if(format.indexOf(mil) > -1){
				var percentages = utility.count(mil,format);
				value = value * Math.pow(1000,percentages);
			}
			
			value = value.toString();
			var rResult = [], lResult = [];
			var numberDecimalSeperatorAt = value.indexOf(".");
			var formatDecimalSeperatorAt = format.indexOf(".");
			var rightFormat = format.substring(formatDecimalSeperatorAt+1);
			var rightNumber = numberDecimalSeperatorAt > -1 ? value.substring(numberDecimalSeperatorAt+1) : undefined; 
			var leftNumber = numberDecimalSeperatorAt > -1 ? value.substring(0,numberDecimalSeperatorAt) : value
			var leftFormat = formatDecimalSeperatorAt > -1 ? format.substring(0,formatDecimalSeperatorAt) : format;
